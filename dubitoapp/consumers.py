...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        if await self.get_player_number(self.player_id) == await self.get_current_turn(self.game_id): # current turn player left during their turn
            await asyncio.sleep(5) # give the player a chance to come back
            if not await self.is_online(self.player_id): # player hasn't come back
    ...
    async def receive(self, text_data):
        ...
        if action == 'reaction': # called when a player clicks on a reaction button
    ...
    async def start_round(self, claimed_card, cards):
        ...
        if(await self.check_card_possession(self.game_id, self.player_id, cards)): # player is illegally trying to play cards they don't have
            return

        if await self.is_locked(self.game_id): # see if someone else got here first
            return
        ...
        await self.check_current_player_online() # routine to handle the case where current player isn't online
    
    async def check_current_player_online(self): # verifies if the current turn player is onine, and if they aren't, passes turn onto next player
        current_turn_player_number = await self.get_current_turn(self.game_id)
        while not await self.is_online(await self.get_player_id_from_number(self.game_id, current_turn_player_number)): # if player isn't online, increment turn
    ...
    async def doubt(self):
        ...
        if await self.get_player_last_turn(self.game_id) == None or await self.get_last_amount_played(self.game_id) == 0 or await self.get_stacked_cards(self.game_id) == None: # trying to doubt before any card has been placed
            return

        if self.player_id == await self.get_player_last_turn(self.game_id): # player doubted themselves
            return
        ...
        print(await self.get_player_name(who_doubted) + " ha dubitato di " + await self.get_player_name(last_player))
    ...
    async def place_cards(self, cards):
        ...
        await self.lock_game(self.game_id) # prevent further action from other player
    ...
    async def send_reaction(self, reaction):
        if reaction not in ['laughing', 'crying', 'drunk', 'smirk', 'thinking', 'angry']: # only allow supported emojis
            return
    ...
    async def verify_or_revoke_victory(self, game_id): # if there was a player with zero cards on last turn, and they still have zero cards after next event, they win
    ...
    async def game_state_handler(self, event):
        if(self.player_id != event['player_id']):
            return
        print("sending state to " + str(event['player_id']))
    ...
    async def reaction_handler(self, event):
        if(self.player_id == event['who']): # don't send reaction back to the player who sent the event
            return
    ...
    async def give_cards_handler(self, event):
        if(self.player_id == event['destination']):
    ...
    async def player_info_handler(self, event):
        if self.player_id != event['dest']: # only send info to the player who joined
            return
        if self.player_id != event['player_id']:
            player_number = await self.get_player_number(event['player_id'])
            player_name = await self.get_player_name(event['player_id'])
    ...
    @database_sync_to_async
    def pass_turn(self, game_id): # increments turn but doesn't update player_last_turn (used for skipping turns of offline players)
        game = Game.objects.get(pk=game_id)
        game.player_current_turn = (game.player_current_turn % game.number_of_players) + 1
        game.save()
    ...
    @database_sync_to_async
    def check_card_possession(self, game_id, player_id, cards): # returns 0 if the requested player has all the requested cards in their hand, 1 otherwise
        cards_counted = {}
        for card in cards: # build a dictionary where to each card code corresponds the amount of that card the player wants to place down
            split_card_str = list(card) # split each character
            ...
            if not card_str in cards_counted:
                cards_counted[card_str] = 1
            else:
                cards_counted[card_str] = cards_counted[card_str] + 1

        for card in cards: # for every card, see if the number of times it appear is greater than the number of copies of it the player has in their hand
            split_card_str = list(card) # split each character
            seed = split_card_str[len(split_card_str)-1] # get last character for seed
            number = card[:-1] # remaining characters are the number
